...
def calculate_sim(path_to_file, w, d, sample_word, sim_name):
    df = pd.DataFrame()
    sim_score = 0
    dict = {}
    for window in w:
        for size in d:
            ...
            model_path = ""
            model_path = path_to_file + model_name
            ...
            if sim_name == 'cos':
                ...
                for i in loaded_wv.wv.vocab:
                    sim_score = manhattan_distance(loaded_wv.wv[sample_word], loaded_wv.wv[i])
                    dict[i] = sim_score
                man_df = pd.DataFrame(sorted(dict.items(), key=lambda item: item[1])[:10],
                ...
                for i in loaded_wv.wv.vocab:
                    ...
                    dict[i] = sim_score
                euc_df = pd.DataFrame(sorted(dict.items(), key=lambda item: item[1])[:10],
                                      columns=[column_name, 'similarty'])
